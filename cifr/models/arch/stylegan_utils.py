...
from .custom_ops import FusedLeakyReLU, fused_leaky_relu, upfirdn2d
...
class EqualConv2d(nn.Module):
    def __init__(
        self, in_channel, out_channel, kernel_size, stride=1, padding=0, bias=True, activation=False
    ):
        super().__init__()
...
class ToRGB(nn.Module):
    def __init__(self, in_channel, style_dim, rgb_dim=3, upsample=True, blur_kernel=[1, 3, 3, 1]):
        super().__init__()
        ...
        self.conv = ModulatedConv2d(in_channel, rgb_dim, 1, style_dim, demodulate=False)
        self.bias = nn.Parameter(torch.zeros(1, rgb_dim, 1, 1))
